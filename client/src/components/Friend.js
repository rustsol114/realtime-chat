...
export default function Friend({ yourFriend, total, activeUrl, active, navigatePage }) {
    return (
        // <Link to={`/${yourFriend.memberUsername}`}>
        <div onPointerDown={() => navigatePage(`/${yourFriend.memberUsername}`)} className={`flex items-center justify-between rounded-xl cursor-pointer py-2.5 px-3 hover:bg-gray-900 ${activeUrl === yourFriend.memberUsername ? 'bg-gray-900' : ''}`}>
